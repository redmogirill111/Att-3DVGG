...
if __name__ == "__main__":
    ###############################################修改此处################################################################
    # 要使用全英文目录，否则cv2.imread读不出来，root_path下就是分类标签
    tongdao = 3
    root_path = r"Y:\8saixuan_jpg\train"
    # root_path = r"Y:\dateset\4Chanel\val"
    result_file = r"Y:\8saixuan_jpg\测试8saixuan_jpgtrain.txt"
    ...
    for index in range(len(fnames)):
        ...
        if tongdao == 3:
            allframes = load_frames3(file_dir=fnames[index])
        else:
            allframes = load_frames4(file_dir=fnames[index])
        ...
        while (allframes.shape[0] >= 17):
            ...
            if tongdao == 3:
                buffer = normalize3(buffer)
            else:
                buffer = normalize4(buffer)
            ...
            # inputs = np.transpose(inputs, (0, 3, 1, 2, 3))
    ...
    with open(result_file, 'a', encoding='utf-8') as f:  # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
        f.write(str(modelName) + "处理" + str(len(fnames)) + "个文件，使用了" + str(
            time_run_over - time_run_start) + "秒 " + "其中，预处理帧需要" + str(
