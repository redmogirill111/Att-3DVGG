...
cut_frame = 1  # 多少帧截一次，自己设置就行
root_path = r"F:\dataset\2paper_video\9xin_zhengli_saixuan_mp4\fin_dataset/"
sou_name = "dataset_mp4"
det_name = "dataset_jpg"
resize_height = 224
resize_width = 224
...
for root_path, dirs, files in os.walk(os.path.join(root_path, sou_name)):  # 这里就填文件夹目录就可以了
    for file in files:
        # 获取文件路径
        if (('.avi' in file) or ("mp4" in file) or (".AVI" in file) or (".MP4" in file) or (".WMA" in file)):
