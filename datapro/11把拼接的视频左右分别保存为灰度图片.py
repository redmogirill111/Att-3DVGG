            ...
            while (True):
                ...
                if current_frame % cut_frame == 0:
                    save_path_left = os.path.join(root_path.replace(sou_name, det_name), file[:-4]).replace(
                        "ALARM_CCDNIR", "Left")
                    save_path_right = os.path.join(root_path.replace(sou_name, det_name), file[:-4]).replace(
                        "ALARM_CCDNIR", "Right")
                    if not os.path.exists(save_path_left):
                        os.makedirs(save_path_left)
                    if not os.path.exists(save_path_right):
                        os.makedirs(save_path_right)
                    frame_name = "0000" + str(file_sp_no) + '.jpg'

                    # 截取帧
                    rgb = cv2.cvtColor(frame[:, :1920], cv2.COLOR_BGR2GRAY)
                    nif = cv2.cvtColor(frame[:, 1920:], cv2.COLOR_BGR2GRAY)

                    # 对帧变换尺寸
                    if (frame_height != resize_height) or (frame_width != resize_width):
                        frame = cv2.resize(frame, (resize_width, resize_height))
                    img_name_full_left = os.path.join(save_path_left, frame_name)
                    img_name_full_right = os.path.join(save_path_right, frame_name)
                    cv2.imwrite(img_name_full_left, img=rgb)
                    cv2.imwrite(img_name_full_right, img=nif)
