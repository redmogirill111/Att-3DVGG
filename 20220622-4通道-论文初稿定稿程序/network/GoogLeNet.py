...
class Inception(nn.Module):
    def __init__(self, in_channel, out_channel_1, out_channel_2, out_channel_3, out_channel_4, **kwargs):
        ...
        self.p1_1 = nn.Conv3d(in_channel, out_channel_1, kernel_size=1)  # 1x1Conv

        self.p2_1 = nn.Conv3d(in_channel, out_channel_2[0], kernel_size=1)  # 1x1Conv
        self.p2_2 = nn.Conv3d(out_channel_2[0], out_channel_2[1], kernel_size=3, padding=1)  # 3x3Conv

        self.p3_1 = nn.Conv3d(in_channel, out_channel_3[0], kernel_size=1)  # 1x1Conv
        self.p3_2 = nn.Conv3d(out_channel_3[0], out_channel_3[1], kernel_size=5, padding=2)  # 5x5Conv

        self.p4_1 = nn.MaxPool3d(kernel_size=3, stride=1, padding=1)  # 3x3 MaxPool
        self.p4_2 = nn.Conv3d(in_channel, out_channel_4, kernel_size=1)  # 1x1 Conv
...
class GoogLeNet(nn.Module):
    ...
    def init_weights(self, layer):
        if type(layer) == nn.Linear or type(layer) == nn.Conv3d:
