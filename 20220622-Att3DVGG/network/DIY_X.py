...
class Depthwise_Separable3d(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(Depthwise_Separable3d, self).__init__()
...
class Xception(nn.Module):
    ...
    def __init__(self, num_classes=3):
        ...
        # Entry flow
        self.conv1 = ConvBlock(4, 32, kernel_size=3, stride=1, bias=False)

        self.conv2 = ConvBlock(32, 32, kernel_size=4, stride=1, bias=False)

        self.conv3 = ConvBlock(32, 64, kernel_size=3, stride=1, bias=False)

        self.block1 = Block(64, 64, reps=2, stride=2, start_with_relu=False)
        self.block2 = Block(64, 128, reps=2, stride=2, start_with_relu=False, grow_first=True)
        self.block3 = Block(128, 128, reps=2, stride=1,
                            start_with_relu=True, grow_first=True, is_last=True)

        # Middle flow
        self.block4 = Block(128, 256, reps=3, stride=1, dilation=middle_block_dilation,
                            start_with_relu=True, grow_first=True)

        # Exit flow
        self.block20 = Block(256, 256, reps=2, stride=1, dilation=exit_block_dilations[0],
                             start_with_relu=True, grow_first=False, is_last=True)

        self.conv4 = SeparableConv3d(256, 256, 3, stride=1, dilation=exit_block_dilations[1])

        self.conv5 = SeparableConv3d(256, 512, 3, stride=1, dilation=exit_block_dilations[1])

        self.conv6 = SeparableConv3d(512, 512, 3, stride=1, dilation=exit_block_dilations[1])
...
if __name__ == "__main__":
    ...
    #
    # g = make_dot(outputs)
    # g.view(r"C:\Users\THHICV\Downloads/DIYX.gv")

    # # 使用netron可视化模型
    # import torch.onnx
    # import netron
    # onnx_path = r"C:\Users\THHICV\Downloads/DIY.onnx"
