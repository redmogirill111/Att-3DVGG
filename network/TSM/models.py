...
from network.TSM.basic_ops import ConsensusModule
...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        if self.dropout == 0:
            ...
            self.new_conv = nn.Conv2d(32, 16, kernel_size=1, stride=1, padding=0)
        std = 0.001
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            if self.is_shift:
                ...
                from network.TSM.temporal_shift import make_temporal_shift
                make_temporal_shift(self.base_model, self.num_segments,
                                    n_div=self.shift_div, place=self.shift_place, temporal_pool=self.temporal_pool)

            if self.non_local:
                print('Adding non-local module...')
                from network.TSM.non_local import make_non_local
            ...
            self.base_model.avgpool = nn.AdaptiveAvgPool2d(1)
            if self.is_shift:
                from network.TSM.temporal_shift import TemporalShift
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.dropout > 0:
            # base_out = self.
            # if base_out.shape[0] != 16:
            #     base_out = self.new_conv(base_out)
            #     pass
            # print("new_fc_bef",base_out.shape)
            base_out = self.new_fc(base_out)
            # print("new_fc_aft",base_out.shape)
